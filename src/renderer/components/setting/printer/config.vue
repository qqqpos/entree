<template>
  <div>
    <header class="nav">
      ...
      <div class="title">
        <h5 v-if="config.type">{{$t('print.type.'+config.type)}}</h5>
        <h3>{{printer}}</h3>
      </div>
      <nav>
    ...
    <template v-else>
      ...
      <toggle :title="$t(config.type === 'hibachi' ?'print.seat':'print.category')" v-model="config.control.categorize" @update="updateCategorize"></toggle>
