  ...
  <div class="popupMask center dark" @click.self="init.reject">
    <ul v-show="!component">
      <li v-show="type !== 'WALK_IN'" @click="change('WALK_IN')">
        ...
        <h3>{{$t('type.WALK_IN')}}</h3>
      </li>
      <li v-show="type !== 'PICK_UP'" @click="change('PICK_UP')">
        <i class="fa fa-phone space-right"></i>
        <h3>{{$t('type.PICK_UP')}}</h3>
      </li>
      <li v-show="type !== 'DELIVERY'" @click="change('DELIVERY')">
        <i class="fa fa-car space-right"></i>
        <h3>{{$t('type.DELIVERY')}}</h3>
      </li>
      <li v-show="type !=='DINE_IN'" @click="change('DINE_IN')">
        <i class="fa fa-utensils space-right"></i>
        <h3>{{$t('type.DINE_IN')}}</h3>
      </li>
      <li v-show="(type === 'DINE_IN' || type === 'HIBACHI') && !app.newTicket" @click="togo">
        <i class="fas fa-walking space-right"></i>
        <h3>{{$t('type.TO_GO')}}</h3>
      </li>
      <li @click="thirdParty" class="thirdParty">
        <i class="fab fa-edge space-right"></i>
        <h3>{{$t('type.THIRD')}}</h3>
      </li>
    </ul>
