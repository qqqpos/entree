...
export const setAppEnvironment = (
  { commit },
  { config, layout, menu, request, orders, tables, templates, books, lastSync }
) => {
...
export const setRequest = ({ commit }, request) =>
  commit(types.SET_REQUEST, request);
export const setOperator = ({ commit }, operator) =>
  commit(types.SET_OPERATOR, operator);
...
export const setViewTable = ({ commit }, table) =>
  commit(types.SET_VIEW_TABLE, table);
export const resetViewTable = ({ commit }) => commit(types.RESET_VIEW_TABLE);
export const setViewOrder = ({ commit }, order) =>
  commit(types.SET_VIEW_ORDER, order);
...
export const addToOrder = ({ commit, getters }, item) =>
  commit(types.ADD_TO_ORDER, {
    item,
    autoStack: getters.config.defaults.autoStackItem
  });
export const pushToOrder = ({ commit }, item) =>
  commit(types.PUSH_TO_ORDER, item);
...
export const moreQty = ({ commit, getters }, requestState) =>
  commit(types.MORE_QTY, {
    requestState,
    matchItemQty: getters.config.defaults.matchItemQty
  });
...
export const emptyChoiceSet = ({ commit }, target) =>
  commit(types.EMPTY_CHOICE_SET, target);
...
export const emptyArchiveOrder = ({ commit }) =>
...
export const setChoiceSet = ({ commit, getters }, subitem) =>
...
export const alterItemOption = ({ commit, getters }, data) => {
  ...
  const subitem = {
    qty: 1,
    single: parseFloat(side.price) || 0,
    price: parseFloat(side.price) || 0,
    usEN: side.usEN,
    zhCN: side.zhCN
  };
